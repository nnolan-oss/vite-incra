...
export const patchConfig = (config: UserConfig, { ignoreWarnings = false } = {}) => {
	...
	config.plugins.unshift({
			...
			void (async () => {
				...
				Object.values(bundle).forEach((fileInfo) => {
					...
					if (!('facadeModuleId' in fileInfo) || !fileInfo.facadeModuleId) {
						...
						if (dictEntry) state.dictionary[fileInfo.name!].realLocationInDist.push(fileInfo.fileName)
						else
		...
		options(options) {
			if (
				state.originalEntries === undefined &&
				options.input &&
				typeof options.input === 'object' &&
				!Array.isArray(options.input) &&
				Object.keys(options.input).length
			)
				state.originalEntries = options.input as Record<string, string>
			const modifiedFile = state.watcherModifiedFile ?? process.env[ENV_MODIFIED_FILE] ?? null
			if (modifiedFile) {
				...
				if (state.originalEntries && matchingItemInEntries) entryName = matchingItemInEntries[0]
