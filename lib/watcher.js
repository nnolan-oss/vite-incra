...
export function startIncrementalBuild({ config, bundleName = 'bundle', watcherIgnoredFiles, watcherUsePolling = true, beforeBuildCallback, watch = true, cachePath, force = false, }) {
    ...
    if (watch) {
        ...
        const ignored = (p) => {
            ...
            for (const pat of ignoredPatterns) {
                ...
                else if (pat instanceof RegExp && pat.test(p))
                    return true;
            }
            return false;
        };
        ...
        if (state.watcherModifiedFile) {
            ...
            console.log('\x1b[90m%s\x1b[0m', changed[0] !== state.watcherModifiedFile
