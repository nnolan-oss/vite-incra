...
export const patchConfig = (config, { ignoreWarnings = false } = {}) => {
    ...
    const existingOutputObj = typeof existingOutput === 'object' && existingOutput !== null && !Array.isArray(existingOutput)
        ? existingOutput
        : {};
        ...
        entryFileNames: ({ facadeModuleId, name }) => {
            ...
            for (const [ext, outExt] of Object.entries(SOURCE_EXT_TO_JS)) {
                if (name.endsWith(ext))
                    return normalizePath(name.slice(0, -ext.length) + outExt);
            }
